...
test.describe('Route Accessibility', () => {
  ...
  test('should access test connection page', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { level: 1 })).toContainText('Frontend/Backend Connection Test');
  });

  test('should access admin dashboard page', async ({ page }) => {
    await page.goto('/admin-dashboard');
    await expect(page).toHaveURL('/admin-dashboard');
    // Dashboard may require authentication in production, but should at least load the route
  });
});
